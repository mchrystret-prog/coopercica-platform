...
export function Leaflets({items,content={}}:{items:Leaflet[];content?:Record<string,string>}){const track=useRef<HTMLDivElement>(null);const move=(dir:number)=>track.current?.scrollBy({left:dir*(track.current.clientWidth*.82),behavior:"smooth"});
  ...
  return <Section id="ofertas" tone="muted" className={styles.section} tabIndex={-1}>
    <Container>
      ...
      <div className={styles.carouselHead}>{items.length>3?<div className={styles.arrows}><button type="button" aria-label="Folhetos anteriores" onClick={()=>move(-1)}>←</button><button type="button" aria-label="Próximos folhetos" onClick={()=>move(1)}>→</button></div>:null}</div><div className={styles.grid} ref={track}>{items.map(item=><Link className={styles.card} href={`/folheteria/${item.slug}`} key={item.id}>
